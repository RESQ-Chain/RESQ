...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 RESQ at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zRESQ send
enum ZerocoinSpendStatus {
    ZRESQ_SPEND_OKAY = 0,                            // No error
    ZRESQ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZRESQ_WALLET_LOCKED = 2,                         // Wallet was locked
    ZRESQ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZRESQ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZRESQ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZRESQ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZRESQ_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZRESQ_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZRESQ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZRESQ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZRESQ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZRESQ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZRESQ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZRESQ_SPENT_USED_ZRESQ = 14,                      // Coin has already been spend
    ZRESQ_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZRESQ_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZResqBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZRESQOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzresqAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzRESQTracker> zresqTracker;
    ...
    void setZWallet(CzRESQWallet* zwallet)
    {
        zwalletMain = zwallet;
        zresqTracker = std::unique_ptr<CzRESQTracker>(new CzRESQTracker(strWalletFile));
    }

    CzRESQWallet* getZWallet() { return zwalletMain; }
    ...
    void setZResqAutoBackups(bool fEnabled)
    {
    ...
    /** zRESQ reset */
    boost::signals2::signal<void()> NotifyzRESQReset;
