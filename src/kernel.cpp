...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZResqStake(spend));
    } else {
        ...
        CResqStake* resqInput = new CResqStake();
        resqInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(resqInput);
    }
