...
CzRESQWallet::CzRESQWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zresq seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRESQSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRESQSeed_deprecated()) {
                    LogPrintf("%s: Updated zRESQ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zresq seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zresq wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRESQWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRESQCount(nCountLastUsed);
    else if (!walletdb.ReadZRESQCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRESQWallet::Lock()
{
    seedMaster = 0;
}

void CzRESQWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRESQWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRESQ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRESQWallet::LoadMintPoolFromDB()
{
...
void CzRESQWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRESQWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRESQWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zresqTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRESQWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRESQ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zresqTracker which also adds to database
    pwalletMain->zresqTracker->Add(dMint, true);
...
void CzRESQWallet::SeedToZRESQ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRESQWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRESQWallet::GenerateDeterministicZRESQ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRESQWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRESQWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRESQWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
