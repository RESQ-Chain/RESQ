...
public:
    ...
    bool WriteZRESQSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRESQSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRESQSeed_deprecated(uint256& seed);
    bool EraseZRESQSeed();
    bool EraseZRESQSeed_deprecated();

    bool WriteZRESQCount(const uint32_t& nCount);
    bool ReadZRESQCount(uint32_t& nCount);
