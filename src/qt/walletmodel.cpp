...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered resq address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 RESQ.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzRESQReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzRESQReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzRESQReset.connect(boost::bind(NotifyzRESQReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzRESQReset.disconnect(boost::bind(NotifyzRESQReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zresqTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
