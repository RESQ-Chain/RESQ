...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeResqAmount"))
        settings.setValue("nAnonymizeResqAmount", 1000);

    nAnonymizeResqAmount = settings.value("nAnonymizeResqAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeResqAmount"))
        SoftSetArg("-anonymizeresqamount", settings.value("nAnonymizeResqAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeResqAmount:
            return QVariant(nAnonymizeResqAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeResqAmount:
            nAnonymizeResqAmount = value.toInt();
            settings.setValue("nAnonymizeResqAmount", nAnonymizeResqAmount);
            emit anonymizeResqAmountChanged(nAnonymizeResqAmount);
            break;
        case CoinControlFeatures:
