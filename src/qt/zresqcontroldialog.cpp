...
#include "ui_zresqcontroldialog.h"
...
std::set<std::string> ZResqControlDialog::setSelectedMints;
std::set<CMintMeta> ZResqControlDialog::setMints;

bool CZResqControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZResqControlDialog::COLUMN_DENOMINATION || column == ZResqControlDialog::COLUMN_VERSION || column == ZResqControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZResqControlDialog::ZResqControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZResqControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZResqControlDialog::~ZResqControlDialog()
{
    delete ui;
}

void ZResqControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZResqControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZResqControlWidgetItem* itemDenom(new CZResqControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZResqControlWidgetItem *itemMint = new CZResqControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zRESQ seed used to mint this zRESQ is not the same as currently hold in the wallet";
            else
...
void ZResqControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZResqControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZResq_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZResqControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZResqControlDialog::GetSelectedMints()
{
...
void ZResqControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
