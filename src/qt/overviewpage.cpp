...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::RESQ)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sRESQPercentage, QString& szRESQPercentage)
{
    int nPrecision = 2;
    ...
    szRESQPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sRESQPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount resqAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // RESQ Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = resqAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // RESQ labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, resqAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelRESQPercent->setText(sPercentage);
    ui->labelzRESQPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zRESQ.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in resq.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in resq.conf");
    }
    ...
    // RESQ Available
    bool showRESQAvailable = settingShowAllBalances || resqAvailableBalance != nTotalBalance;
    bool showWatchOnlyRESQAvailable = showRESQAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showRESQAvailable || showWatchOnlyRESQAvailable);
    ui->labelBalance->setVisible(showRESQAvailable || showWatchOnlyRESQAvailable);
    ui->labelWatchAvailable->setVisible(showRESQAvailable && showWatchOnly);

    // RESQ Pending
    bool showRESQPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyRESQPending = showRESQPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showRESQPending || showWatchOnlyRESQPending);
    ui->labelUnconfirmed->setVisible(showRESQPending || showWatchOnlyRESQPending);
    ui->labelWatchPending->setVisible(showRESQPending && showWatchOnly);
    ...
    // RESQ Locked
    bool showRESQLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyRESQLocked = showRESQLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showRESQLocked || showWatchOnlyRESQLocked);
    ui->labelLockedBalance->setVisible(showRESQLocked || showWatchOnlyRESQLocked);
    ui->labelWatchLocked->setVisible(showRESQLocked && showWatchOnly);

    // zRESQ
    bool showzRESQAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzRESQUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzRESQImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzRESQAvailable);
    ui->labelzBalanceMatureText->setVisible(showzRESQAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzRESQUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzRESQUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzRESQImmature);
    ui->labelzBalanceImmatureText->setVisible(showzRESQImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelRESQPercent->setVisible(showPercentages);
    ui->labelzRESQPercent->setVisible(showPercentages);
