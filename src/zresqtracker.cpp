...
CzRESQTracker::CzRESQTracker(std::string strWalletFile)
{
...
CzRESQTracker::~CzRESQTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRESQTracker::Init()
{
...
bool CzRESQTracker::Archive(CMintMeta& meta)
{
...
bool CzRESQTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRESQTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRESQTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRESQTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzRESQTracker::GetSerialHashes()
{
...
CAmount CzRESQTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRESQTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRESQTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRESQTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRESQTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRESQTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRESQTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRESQTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRESQTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRESQTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRESQTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRESQWallet* zRESQWallet)
{
    bool iszRESQWalletInitialized = (NULL != zRESQWallet);
    CMintMeta meta;
    ...
    if (! iszRESQWalletInitialized)
        zRESQWallet = new CzRESQWallet(strWalletFile);
    meta.isSeedCorrect = zRESQWallet->CheckSeed(dMint);
    if (! iszRESQWalletInitialized)
        delete zRESQWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRESQTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRESQTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRESQTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRESQTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRESQTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRESQTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRESQWallet* zRESQWallet = new CzRESQWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zRESQWallet);
        }
        delete zRESQWallet;
        LogPrint("zero", "%s: added %d dzresq from DB\n", __func__, listDeterministicDB.size());
    }
