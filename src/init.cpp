...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "resq.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "resqd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the RESQ and zRESQ money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in RESQ/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in RESQ/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, resq, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in RESQ/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all RESQ specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-resqstake=<n>", strprintf(_("Enable or disable staking functionality for RESQ inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zresqstake=<n>", strprintf(_("Enable or disable staking functionality for zRESQ inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzresq=<n>", strprintf(_("Enable automatic wallet backups triggered after each zRESQ minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zresqbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zRESQ backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeresqamount=<n>     " + strprintf(_("Keep N RESQ anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The RESQ Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that RESQ is running in a usable environment with all
...
#endif
    // Make sure only a single RESQ process is using the data directory.
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //RESQ: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //RESQ specific: zerocoin and spork DB's
                ...
                // RESQ: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzRESQWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zRESQWallet
        uiInterface.InitMessage(_("Syncing zRESQ wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZResqBackups = GetBoolArg("-backupzresq", true);
        pwalletMain->setZResqAutoBackups(fEnableZResqBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zresqTracker->Init();
...
//    }
//
//    nAnonymizeResqAmount = GetArg("-anonymizeresqamount", 0);
//    if (nAnonymizeResqAmount > 999999) nAnonymizeResqAmount = 999999;
//    if (nAnonymizeResqAmount < 2) nAnonymizeResqAmount = 2;
    ...
    LogPrintf("Anonymize RESQ Amount %d\n", nAnonymizeResqAmount);
    ...
    /* Denominations
       ...
       For example:
       1RESQ+1000 == (.1RESQ+100)*10
       10RESQ+10000 == (1RESQ+1000)*10
    */
